...
# The function *parse_10k_filing()* parses 10-K forms to extract the following sections: business description, business risk, and management discussion and analysis. The function takes two arguments, a file-path and a number indicating the section, and returns a list with the requested sections. Current options are **0(All), 1(Business), 2(Risk), 3(MDA).**
# 
# Caveats:
# The function *parse_10k_filing()* is a parser. You need to feed a file-path into it. There are many python and r packages to get a direct link to the fillings.
# 
